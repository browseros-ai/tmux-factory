...
# ── 4. opus default validates claudeo, while overrides bypass validation ───────
grep -qF -- 'CLAUDE_CMD="claudeo"' "$SCRIPT" \
  || fail "max effort must default to claudeo"
grep -qF -- "type claudeo" "$SCRIPT" \
  || fail "max effort validation must check claudeo"
if grep -qF -- 'CLAUDE_CMD="claudex"' "$SCRIPT"; then
  fail "opus launcher must not default to claudex"
fi

VALIDATE_LOG="$TMP/validate-agent-command.log"
export VALIDATE_LOG
AGENT_SHELL="$TMP/agent-shell"
cat >"$AGENT_SHELL" <<'SH'
#!/usr/bin/env bash
printf '%s\n' "$*" >>"$VALIDATE_LOG"
case "$*" in
...
: >"$VALIDATE_LOG"
EFFORT=max
unset SF_CLAUDEGO_CMD
validate_agent_command || fail "max effort validation rejected fake claudeo"
grep -qF -- 'type claudeo' "$VALIDATE_LOG" \
  || fail "max effort validation did not check claudeo"

: >"$VALIDATE_LOG"
EFFORT=medium
unset SF_CLAUDEGO_CMD
validate_agent_command || fail "medium effort validation rejected fake claude"
grep -qF -- 'type claude >/dev/null' "$VALIDATE_LOG" \
  || fail "non-max effort validation did not check claude"

: >"$VALIDATE_LOG"
EFFORT=max
SF_CLAUDEGO_CMD="custom-claude-command"
validate_agent_command || fail "SF_CLAUDEGO_CMD should bypass command validation"
[ ! -s "$VALIDATE_LOG" ] || fail "SF_CLAUDEGO_CMD should not call the agent shell"
unset SF_CLAUDEGO_CMD

# ── 5. dry-run preview shows the same ordering (worktree + guard before spawn) ──
...
# ── 6. auto-attach uses the tfmux-native `tfmux attach`, not a helper script ────
...
# ── 7. cleanup guidance points at the bundled factory-cleanup.sh helper ───────
