#!/usr/bin/env bash
# test-launch-order.sh — guards the worktree-first invariant for codex-go.sh:
# the worktree must be created and its .llm initialized BEFORE codex is spawned,
...
line_of() {
  ...
  [ -n "$n" ] || fail "anchor not found in codex-go.sh: $1"
  printf '%s' "$n"
}
...
SF_CODEX_TEST_HELPERS=1 source "$SCRIPT"
TMP="$(mktemp -d)"
trap 'rm -rf "$TMP"' EXIT
...
# (codex-go's preflight also needs a real git remote, which the skl repo has).
# If the preview can't be produced, skip rather than emit a false failure — parts
# 1-3 already pin the invariant deterministically.
STUB="$(mktemp -d)"
for b in wt dotllm tfmux gh tmux; do printf '#!/bin/sh\nexit 0\n' >"$STUB/$b"; chmod +x "$STUB/$b"; done
...
if [ -n "$dry" ]; then
  ...
  printf 'note: codex-go --dry-run unavailable in this environment; skipped dry-run ordering check\n'
fi
...
printf '%s\n' "$cleanup_dry" | grep -F 'cleanup after merged PR:' | grep -qF -- "$SCRIPT_DIR/factory-cleanup.sh --session" \
  || fail "cleanup command must prefer the bundled helper over a stale global source helper"

printf 'ok - codex-go launch order (worktree-first invariant)\n'
