#!/usr/bin/env bash
# test-no-git.sh — verifies codex-go.sh degrades to an in-place launch outside
# git: no worktree/branch/PR loop, cwd is the firing directory, dotllm is
# best-effort, and tfmux still delivers the prompt + ping instruction.
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SCRIPT="$SCRIPT_DIR/codex-go.sh"
...
printf '%s\n' "$dry" | grep -qF 'no git repo: running in place, no PR/merge' \
  || fail "dry-run did not disclose no-git/no-PR mode"
...
if printf '%s\n' "$dry" | grep -qF '$sf-auto'; then
  fail "dry-run preview sent sf-auto outside git"
fi
...
printf '%s\n' "$out" | grep -qF 'no git repo: running in place, no PR/merge' \
  || fail "launch output did not disclose no-git/no-PR mode"
...
grep -qF "tfmux <send> <sf_plain_run_codex>" "$LOG" \
  || fail "launcher did not deliver the task through tfmux"
grep -qF 'create hello.txt containing hi, then ping' "$LOG" \
  || fail "tfmux send did not include the plain task"
grep -qF 'Do not use the software-factory PR/merge loop' "$LOG" \
  || fail "tfmux send did not instruct codex to skip the PR/merge loop"
if grep -qF '$sf-auto' "$LOG"; then
  fail "tfmux send mentioned sf-auto outside git"
fi
...
printf 'ok - codex-go no-git mode\n'
